...
Как только мы запускаем наш `penspin_calculate.exe` файл, появляется 2 окна: *консоль* и окно "PenSpin Calculate". В *консоли* мы ничего 
 писать не будем и не сможем, там будет только выводиться что-то, что нам будет нужно.

Второе окно с названием "PenSpin Calculate" это то окно, в котором мы и будем работать, там есть: *строка для ввода команды*, *кнопка* и *панель*, на которой 
...
Попробуем написать в *строку для ввода* команду `add dgg 4.3 5.15 #F1C40F` и нажать **<Enter>**. Во-первых, строка должна стать пустой, а на *панели* dgg
должен был появиться прямоугольника привычного для ДГГ цвета ~**_мочи_**~. Этот прямоугольник и обозначает деталь, а именно ДГГ с массой 4.3 грамма и 
...
если отрицательно, то влево. Также можно задать вместо аргумента **<_расстояние_>** не число, а параметр *center*, и тогда деталь переместится так, что её центр совпадет 
...
- `listed print` - выводит в консоль все детали с названиями и характеристиками, которые содержатся в файле *listed.json* 
